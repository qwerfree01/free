...
def subscribe():
	if request.method=="POST":
		...
		elif change_id == "QuotaRedistribution":
			return render_template_string('''
			<!DOCTYPE html>
			<html lang="en">
			<head>
			    ...
			    <h2><font color="red">Designed by sherif omar</font></h2>
			    <p>إعادة تقسم فليكس</p>
			    
			    <form method="post" action="/Redistribution">
			    	...
			    	<button><font color="green">تقسيم</font></button>
			    	<br><br>~~
...
def accept():
	if request.method=="POST":
		...
		try:
			...
			response = requests.patch(url, headers=head, data=data).json()
			print(response)
...
@app.route("/Redistribution",methods=["POST","GET"])
def Redistribution():
	if request.method=="POST":
		owner=request.form["owner"]
		password=request.form["pas"]
		...
		try:
			...
			data={"category":[{"listHierarchyId":"TemplateID","value":"47"}],"createdBy":{"value":"MobileApp"},"parts":{"characteristicsValue":{"characteristicsValue":[{"characteristicName":"quotaDist1","type":"percentage","value":"40"}]},"member":[{"id":[{"schemeName":"MSISDN","value":owner}],"type":"Owner"},{"id":[{"schemeName":"MSISDN","value":member}],"type":"Member"}]},"type":"QuotaRedistribution"}
			#print(data)
			response = requests.patch(url, headers=head, json=data).json()
			print(response)
			
			if str(response)=="{}":
				return '''<center><br><h1>تم تقسيم 5200 فليكس</h1></center>'''
				
			else:
				return '''<center><br><h1>لم يتم التقسيم  راجع الابلكيش </h1></center>'''
